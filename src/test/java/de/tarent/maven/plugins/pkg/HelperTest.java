...
public class HelperTest extends AbstractMvnPkgPluginTestCase {
	...
	public void setUp() throws Exception {
		...
		// Helper needs a PackageMap instance and that cannot be changed
		// afterwards. However
		// for testing purposes we want to be able to change every bit of the
		// implementation.
		// For this the following instance of PackageMap delegates all calls to
		// the 'packageMap'
		// instance. The test implementor can then set her *own* instance to
		// packageMap. The
		// default used above is for those who are not interested in changing
		// the packageMap
		// at all.
		PackageMap delegatingPackageMap = new PackageMap(null, null, homedir,
				null) {
			...
			@Override
			public void iterateDependencyArtifacts(Log l,
					Collection<Artifact> deps, Visitor v,
					boolean bundleNonExisting) {
				packageMap.iterateDependencyArtifacts(l, deps, v,
						bundleNonExisting);
			}
		};

		helper = new Helper();
		helper.init(packaging, delegatingPackageMap, targetConfiguration,
				resolvedConfigurations, "foobar");
		previousfilefound = false;
		if (f.exists()) {
			FileUtils
					.moveFile(f, new File(homedir + "/.rpmmacros_Test_backup"));
	...
	public void tearDown() throws Exception {
		...
		if (previousfilefound) {
			f.delete();
			FileUtils
					.moveFile(new File(homedir + "/.rpmmacros_Test_backup"), f);
		}
	}

	@Test
	public void creatingRpmmacrosfileWithoutMaintainerAndRemovingSuccessfully()
	...
	@Test
	public void creatingRpmmacrosfileWitMaintainerAndRemovingSuccessfully()
			throws IOException, MojoExecutionException {
		...
		Assert.assertTrue("String not found",
				filecontains(f, "%_gpg_name       Dummy maintainer"));
		helper.restoreRpmMacrosFileBackup(null);
	}

	/**
	 * Checks whether macro file generation works without exception with an
	 * unconfigured {@link TargetConfiguration}.
	 * 
	 * @throws IOException
	 * @throws MojoExecutionException
	 */
	public void testCreatingRpmmacrosfileWithoutBaseDir() throws IOException,
	...
	@Test
	public void prepareInitialDirectoriesSuccesfully_RPM()
		...
		Assert.assertEquals(new File(helper.getBasePkgDir(), "/BUILD"),
				helper.getBaseBuildDir());
		Assert.assertEquals(new File(helper.getBasePkgDir(), "/SPECS"),
				helper.getBaseSpecsDir());
		FileUtils.deleteDirectory(base);
	}

	@Test(expected = NullPointerException.class)
	public void prepareInitialDirectoriesWithoutBaseOrTempRootThrowsException()
			throws MojoExecutionException {
		helper.prepareInitialDirectories();
	}
	...
	public void getVersionReturnsPackagingVersion_RPM() throws Exception {
		...
		Assert.assertTrue(helper.getPackageVersion().contains(
	...
	public void testVersion_versionsuffix_revision_DEB() throws Exception {
		...
		String expected = String.format("%s-0%s-%s",
				packaging.project.getVersion(), vs, rs);
		Assert.assertEquals(expected, helper.getPackageVersion());
	}
	...
	public void getDstArtifactFileReturnsgetBaseBuildDirAndgetTargetArtifactFiletoStringIfNotSet_RPM() {
		...
		Assert.assertEquals(helper.getBaseBuildDir() + "/"
				+ helper.getTargetArtifactFile().toString(), helper
				.getDstArtifactFile().toString());
	}
	 ...
	 */
	@Test
	public void testCreateDependencyLine_defaultDependencyLine()
			throws Exception {
		...
		packageMap = new PackageMap(null, null, "default", null) {
			...
			@Override
			public boolean isDebianNaming() {
				// Not really necessary for this test but the test string
				// contains
	...
	/**
	 * Tests the successful addition of resolved dependencies to the dependency
	 * line created by {@link Helper#createDependencyLine}.
	 * 
	 * @throws Exception
	 */
	@Test
