...
public class RepreproDeployer implements IPkgUploader {
	...
	/**
	 * Defines the repository location to use for reprepro. For use on the
	 * command-line.
	 * 
	 */
	...
	public RepreproDeployer(WorkspaceSession ws, String repo) {
		...
		this.packageFileName = ws.getHelper()
				.getPackageFileNameWithoutExtension();
		this.distro = ws.getHelper().getChosenDistro();
		this.changesFile = new File(base, packageFileName + ".changes");
	}

	/**
	 * Deploys the .deb to a local apt repository. Currently only reprepro is
	 * supported.
	 * 
	...
	public void uploadPackage() throws MojoExecutionException {
		...
		l.info("Deploying " + base.getAbsolutePath() + "/" + packageFileName
				+ ".deb");
	...
	protected String[] generateCommand() {
		return new String[] { REPREPRO, "-b" + repo, "include", distro,
	...
	public void checkPrerequisites() throws MojoExecutionException {
		...
		if (repo.isEmpty()) {
			throw new MojoExecutionException(
					"Repository location can not be empty. Aborting.");
		}

		if (!packagingType.equals("deb")) {
			throw new MojoExecutionException("Uploading packaging-type '"
					+ packagingType + "' to reprepro not supported.");
		}

		if (!changesFile.exists()) {
			throw new MojoExecutionException("Changes file for '"
					+ packageFileName + "' does not exist.");
		}
	}
