...
public class AotCompileUtils
{
  ...
  throws MojoExecutionException
  {
    Utils.exec(new String[] { GCJ_EXECUTABLE, "-dumpversion" },
               GCJ_EXECUTABLE + " returned with an error. Check your installation!",
               GCJ_EXECUTABLE + " is not available on this system. Check your installation!");

    Utils.exec(new String[] { GCJ_DBTOOL_EXECUTABLE, "-p" },
               GCJ_DBTOOL_EXECUTABLE + " returned with an error. Check your installation!",
               GCJ_DBTOOL_EXECUTABLE + " is not available on this system. Check your installation!");
  }
  
  /**
   * Does a compilation and classmap generation step for a set of artifacts. It returns
   * the amount of bytes claimed by the classmaps and the aot binaries.
   *  
   * @param l A Log instance.
   * @param artifacts A set of Artifact instances.
   * @param aotDstDir The directory where the aot binaries should be put.
   * @param extension The file extension to be appended to each binary (e.g. ".jar.so")
   * @param aotDstClassmapDir The directory where the classmaps should be put.
    ...
    throws MojoExecutionException
  {
	...
	while (ite.hasNext())
	{
		...
		File classmap = new File(aotDstClassmapDir, a.getArtifactId() + ".db");
	    compile(l, src, dst);
  ...
  Utils.exec(new String[] {GCJ_EXECUTABLE,
                           "-O2",
                           "-g",
                           "-Wl,-Bsymbolic",
                           "-shared",
      ...
      throws MojoExecutionException
  {
    ...
    Utils.exec(new String[] {GCJ_DBTOOL_EXECUTABLE,
               ...
               GCJ_DBTOOL_EXECUTABLE + " returned with an error.",
               "IOException while creating classmap file.");
    
    String dsoName = (overridePath.length() > 0)
                     ? overridePath + "/" + binary.getName()
                     : binary.getName();

    l.info("filling classmap file");
    Utils.exec(new String[] {GCJ_DBTOOL_EXECUTABLE,
                             "-f", classmap.getAbsolutePath(),
                             jar.getAbsolutePath(),
                             dsoName },
                GCJ_DBTOOL_EXECUTABLE + " returned with an error.",
                "IOException while creating classmap file.");
      ...
      throws MojoExecutionException
  {
    ...
    Utils.storeInputStream(
                          ...
                          postinstFile,
                          "IOException while depositing the postinst file.");
