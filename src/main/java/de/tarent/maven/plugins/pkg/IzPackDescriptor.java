...
class IzPackDescriptor
{
  ...
  void finish(File dest, String exceptionMsg)
  throws MojoExecutionException
  {
	...
	if (appPackNode == null)
	 {
	   ...
	   for (int i=0; i<size; i++)
	     {
	       if (packs.item(i).getAttributes().getNamedItem("required").getNodeValue().equals("yes"))
	        {
  ...
  /**
   * Adds the information to let IzPack install a Unix wrapper script. The basic
   * installation information should be added to the pack which denotes the application'
   * base installation. In order to find it a node with the XML id identical to
   * <code>APP_PACK_ID</code> is searched. If it can be found its required parameter
   * is set to "yes". If it cannot be found the method searches for the first node
   * whose "required" attribute is set to "yes". If the search fails a
   * <code>MojoExecutionException</code> is thrown.
   * 
   * Additionally another pack is created which allows installing the start script to
   * a system-wide location. If the XML contains a pack whose XML id is identical to
   * <code>ROOT_INSTALL_PACK_ID</code> this method will use that pack only adds missing
   * information.
   * 
   * The root install pack is supposed to have a "name", "required", "preselected" and
   * "os" attribute. If any of those is given, it will not be overwritten. Missing
   * entries will be added with default values.
   * 
   * The root install pack is supposed to have a description, file, parsable and executable
   * child node. Any existing child node is not touched and missing elements are added
   * automatically.
   * 
   * @param scriptName
  ...
  throws MojoExecutionException
  {
    ...
    attrs.setNamedItem(createAttribute("targetfile", "$INSTALL_PATH/" + scriptName));
    attrs.setNamedItem(createAttribute("os", "unix"));
    ...
    if (!childExists(rootInstallPackNode, "description"))
      {
        ...
        descriptionNode.setTextContent("Puts the wrapper script '" + scriptName + "' in /usr/local/bin. For this operation to succeed the installer must be run with root privileges!");
    ...
    if (!childExists(rootInstallPackNode, "parsable"))
      {
        ...
        attrs = parsableNode.getAttributes();
        attrs.setNamedItem(createAttribute("type", "shell"));
        attrs.setNamedItem(createAttribute("targetfile", "/usr/local/bin/" + scriptName));
  ...
  throws MojoExecutionException
  {
    ...
    attrs = parsableNode.getAttributes();
    attrs.setNamedItem(createAttribute("type", "shell"));
    attrs.setNamedItem(createAttribute("targetfile", "$INSTALL_PATH/" + starterDir + "/" + classpathPropertiesFile));
    requiredPackNode.appendChild(parsableNode);
  }
  
  /** Finds the aot pack by XML id and removes it.
   * 
   * This makes it possible that someone adds information for the aot
   * pack in the descriptor but disables it in the POM.
   *
   * The obvious need for this is that GCJ is sometimes incapable of
   ...
   */
  void addAotPack(String aotRoot, String fixClassmapJar, String fixClassmapScript)
  {
    ...
    if (packNode == null)
      {
        ...
        Node descriptionNode = doc.createElement("description");
        descriptionNode.setTextContent("Natively compiled version of the application.");
        packNode.appendChild(descriptionNode);
      }
    
    // Adds a node which causes copying the folder containing all the aot stuff.
    ...
    // Adds a node which causes the copying of the fixclassmap program's jar file.
    ...
    // Adds a node which causes the copying the fixclassmap program's wrapper script file.
    ...
    // Adds a node which causes the The fixclassmap program's wrapper script file.
    Node parsableNode = doc.createElement("parsable");
    attrs = parsableNode.getAttributes();
    attrs.setNamedItem(createAttribute("type", "shell"));
    attrs.setNamedItem(createAttribute("targetfile", "$INSTALL_PATH/" + fixClassmapScript));
    attrs.setNamedItem(createAttribute("os", "unix"));
