...
/**
 * A <code>Mapping</code> is the datatype that describes a target distribution
 * (id, label, packaging system, some path names) and the important mapping
 * between Maven2 artifacts and the distribution's packages.
 * 
...
class Mapping {
	 ...
	 * 
	 * All properties from the child that are set are taken. The others are
	 * taken from the parent.
	 * 
	 * In case of the mapping itself: The entries from the parent are cloned and
	 * the ones from the child are added to it (possibly replacing existing
	 * entries).
	 * 
	 * @param child
	 * @param parent
	...
	Mapping(Mapping child, Mapping parent) {
		...
		// These values may be null. If the merging has been done from the root
		// to the child
		// they will be non-null for the parent however.
		label = (child.label != null) ? child.label : parent.label;
		packaging = (child.packaging != null) ? child.packaging
				: parent.packaging;
		repoName = (child.repoName != null) ? child.repoName : parent.repoName;
		debianNaming = (child.debianNaming != null) ? child.debianNaming
				: parent.debianNaming;
		defaultJarPath = (child.defaultJarPath != null) ? child.defaultJarPath
				: parent.defaultJarPath;
		defaultBinPath = (child.defaultBinPath != null) ? child.defaultBinPath
				: parent.defaultBinPath;
		defaultSBinPath = (child.defaultSBinPath != null) ? child.defaultSBinPath
				: parent.defaultSBinPath;
		defaultJNIPath = (child.defaultJNIPath != null) ? child.defaultJNIPath
				: parent.defaultJNIPath;
		defaultDependencyLine = (child.defaultDependencyLine != null) ? child.defaultDependencyLine
				: parent.defaultDependencyLine;

		// Make a deep copy of the entry map contents. Otherwise parent and
		// 'this' would
		// share the HashSet<Entry> instance causing damaging effects to the
		// parent when stuff
		...
		for (Map.Entry<String, HashSet<Entry>> e : child.entryMap.entrySet()) {
			...
			// 'tset' (the child's Entry instances) replace those that are
			// equivalent to
			// those in 'set' (the ones inherited from parent). We assume that
			// the child's
			// instances are newer.
			// Entry is coded in a way that only the artifactSpec and
			// versionRange properties
			// are needed to decide whether two instances are equal (same goes
			// for hashCode()).
			//
			// As such we do a remove operation first, which will delete the
			// equivalent instances.
			// Afterwards we add the whole set from child.
			//
			// The addAll() implementation alone would not *replace* the
			// existing instances.
