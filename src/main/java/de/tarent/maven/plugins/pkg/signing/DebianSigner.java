...
/**
 * Generates a changes-file for the package generated by the pkg-goal and signs
 * it.
 * 
...
public class DebianSigner {
	...
	/**
	 * The command to use for generating a date-string which complies to the
	...
	/**
	 * Constructor for use of this class within other classes and not just as a
	 * maven goal
	 * 
	 * @param tc
	 * @param ph
	 ...
	 */
	public DebianSigner(WorkspaceSession ws, boolean awaitUserInput)
			throws MojoExecutionException {
		...
		this.packageFileNameWithoutExtension = ws.getHelper()
	 ...
	 * <p>
	 * If set to true it will show user prompts for some steps.
	 * </p>
	 * <p>
	 * Default is true
	 * </p>
	...
	protected void signPackage(Log l, File base) throws MojoExecutionException {
		...
		File pathToChangesFile = new File(base, packageFileNameWithoutExtension
				+ ".changes");
		/**
		...
		if (!awaitUserInput && userInput != null) {
			Utils.exec(new String[] { "gpg", "--no-tty", "--passphrase",
					userInput, "--default-key", maintainer, "--no-use-agent",
					"--yes", "--clearsign", pathToChangesFile.getName() },
					base, "Signing the changes-file failed.",
					"Error signing the changes-file.");

			try {
				FileUtils.copyFile(new File(pathToChangesFile + ".asc"),
						pathToChangesFile);
			} catch (IOException e) {
			...
			Utils.exec(
					new String[] { signCmd, pathToChangesFile.getAbsolutePath() },
					base, "Signing the changes-file failed.",
					"Error signing the changes-file.");
		}
	 ...
	 * generate a files-list.
	 * 
	 * @param l
	 * @param base
	 * @throws MojoExecutionException
	 */
	protected void generateFileList(Log l, File base)
			throws MojoExecutionException {
		...
		Utils.createFile(tempControlFile,
				"Could not create temporary debian/control file for "
						+ filesGenCmd);

		File pathToFileListFile = new File(tempRoot, "files");

		Utils.exec(
				new String[] { filesGenCmd,
						"-f" + pathToFileListFile.getAbsolutePath(),
						packageFileName, section, "optional" }, base,
				"Generating file-list failed.", "Error creating the file-list.");

		try {
			FileUtils.deleteDirectory(tempControlDir);
		} catch (IOException e) {
			throw new MojoExecutionException(
					"Could not delte temporary debian/ directory for "
							+ filesGenCmd);
		}
	}
	 ...
	 * @throws MojoExecutionException
	 */
	protected String getRFC2822Date(Log l, File base)
			throws MojoExecutionException {
		l.info("calling 'date -R' to get RFC‐2822 date");

		InputStream processOutput = Utils.exec(new String[] { rfc2822DateCmd,
				"-R" }, base, "Generating RFC-2822 date failed",
				"Error generating RFC-2822 date.");

		if (processOutput != null) {
			try {
				return new BufferedReader(new InputStreamReader(processOutput))
						.readLine();
			} catch (IOException e) {
				throw new MojoExecutionException(
						"Generating RFC-2822 date failed. ", e);
			}
		} else {
			throw new MojoExecutionException(
					"Generating RFC-2822 date failed (No output from "
							+ rfc2822DateCmd + ").");
		}
	}
	 ...
	 * @throws MojoExecutionException
	 */
	protected void generateChangelogFile(Log l, File base)
			throws MojoExecutionException {
		...
		try {
			...
			throw new MojoExecutionException(
					"IOException while creating changelog file.", e);
		}
	}

	/**
	 * Prompts the user for a list of changes since last deployment of the
	 * package.
	 * 
	 * @return a String-array containing the changes. The size of the array
	 *         corresponds to the amount of changes.
	 * @throws MojoExecutionException
	 */
	protected List<String> promptForChanges() throws MojoExecutionException {
		...
		if (awaitUserInput) {
			System.out
					.println("Please type in your changes since last deployment of this package. One change per line. If you are ready just make an empty line.");
			...
			try {
				...
				throw new MojoExecutionException("Error getting user-input ",
						excp);
			}
			if (changes.size() == 0) {
	...
	protected void generateChangesFile(Log l, File base)
			throws MojoExecutionException {
		...
		InputStream processOutput = Utils.exec(new String[] { changesGenCmd,
				"-b", "-c" + pathToControlFile.getAbsolutePath(),
				"-l" + pathToChangelogFile.getAbsolutePath(),
				"-f" + pathToFileListFile.getAbsolutePath(), "-m" + maintainer,
				"-e" + maintainer }, tempRoot,
				"Generating changes-file failed",
				"Error creating the changes-file.");

		// Store output of the executed process into a .changes-file
		if (processOutput != null) {
			Utils.storeInputStream(processOutput, pathToChangesFile,
					"Error when storing the changes-file.");
		} else {
	...
	/**
	 * Generates a control-file containing a 'Source' line which is needed for
	 ...
	 * @throws MojoExecutionException
	 */
	protected void generateSourceControlFile(Log l, File base)
			throws MojoExecutionException {
		File pathToControlFile = new File(tempRoot, "control");

		l.info("creating source-control file: "
				+ pathToControlFile.getAbsolutePath());
