...
/**
 * Representation of a path, ie. something that is used for specifing a
 * classpath, a boot classpath.
 * 
 * <p>
 * It consists of entries. By using the methods {@link #toUnixPath()} and
 * {@link #toWindowPath()} the path can be translated into something which a
...
public class Path implements Iterable<String> {
	...
	private static String fixForUnix(String entry) {
		...
		String e = UNIX_SPACE_PATTERN.matcher(entry).replaceAll("\\\\ ");
	...
	private static String fixForWindows(String entry) {
		...
		String e = WINDOWS_SLASH_PATTERN.matcher(entry).replaceAll("\\\\");
